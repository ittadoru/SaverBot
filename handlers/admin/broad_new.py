...
async def _send_task(bot: Bot, admin_id: int, data: dict):
    ...
    if not user_ids:
        ...
        return
    total = len(user_ids)
    logger.info(f"Начинается рассылка 'trial_broadcast' для {total} пользователей.")
    ...
    logger.info("Рассылка 'trial_broadcast' завершена. Отправлено=%s Ошибок=%s", sent, failed)
