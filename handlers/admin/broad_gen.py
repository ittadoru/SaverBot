...
def _keyboard(send_button_label: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📝 Текст", callback_data="broadcast:set_text"),
         ...
         InlineKeyboardButton(text="🎬 Медиа", callback_data="broadcast:set_media")],
        [InlineKeyboardButton(text="👀 Предпросмотр", callback_data="broadcast:preview")],
        [InlineKeyboardButton(text=send_button_label, callback_data="broadcast:send"),
         InlineKeyboardButton(text="🚫 Стоп", callback_data="broadcast:cancel")],
    ])
...
async def _send_task(bot: Bot, admin_id: int, data: dict):
    ...
    if not user_ids:
        ...
        return
    total = len(user_ids)
    logger.info("📢 [BROADCAST] Начинается рассылка для %s пользователей.", total)
    ...
    for user_id in user_ids:
        try:
            ...
            error_text = str(e)
            logger.info("🚫 [BROADCAST] TelegramAPIError: %s", error_text)
            failed += 1
        except Exception as e:
            logger.error("🚫 [BROADCAST] Неизвестная ошибка при отправке пользователю %s: %s", user_id, e, exc_info=True)
            failed += 1
    ...
    logger.info("✅ [BROADCAST] Рассылка завершена. Отправлено=%s Ошибок=%s", sent, failed)
